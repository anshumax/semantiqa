...
export interface MysqlSchemaColumn {
  name: string;
  type: string;
  nullable: boolean;
...
export interface MysqlSchemaSnapshot {
  tables: MysqlSchemaTable[];
}

export function persistMysqlSnapshot(db: Database, snapshot: MysqlSchemaSnapshot) {
  ...
  const transaction = db.transaction(() => {
    for (const table of snapshot.tables) {
      const tableId = `mysql.${table.schema}.${table.name}`;
      insertNode.run({
        id: tableId,
        type: 'table',
